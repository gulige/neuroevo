...
metabolics([],Acc)->
%	io:format("Time:~p Avatar_Count:~p~n",[util:now(),length(Acc)]),
	Acc.
...
check_borders(Avatar,[{XMin,XMax},{YMin,YMax}])->
	...
	case {DX,DY} of
		{0,0} ->
			Avatar;
		{DX,DY} ->
			U_Objects=[{ObjName,Id,Color,{PX+DX,PY+DY},[{X_+DX,Y_+DY}||{X_,Y_}<-Coords],P}||{ObjName,Id,Color,{PX,PY},Coords,P}<-Avatar#avatar.objects],
...
%	io:format("Respawning:~p~n",[A]),
	...
	OAvatars = [A_ || A_ <- Avatars, (A#avatar.type==rock) or (A#avatar.type==pillar) or (A#avatar.type==fire_pit)],
