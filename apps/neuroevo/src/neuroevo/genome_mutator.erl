...
add_actuator(Agent_Id)->%TODO: There should be a preference towards adding actuators not yet used.
	...
	case morphology:get_Actuators(Morphology)--[(genotype:read({actuator,A_Id}))#actuator{cx_id=undefined,id=undefined,fanin_ids=[],generation=undefined} || A_Id<-A_Ids] of
		...
		Available_Actuators ->
			io:format("Used actuators:~p~n Filtered avaialble actuators:~p~n",[morphology:get_Actuators(Morphology),[(genotype:read({actuator,A_Id}))#actuator{cx_id=undefined,id=undefined,fanin_ids=[],generation=undefined} || A_Id<-A_Ids]]),
			io:format("Available_Actutors:~p~n",[Available_Actuators]),
